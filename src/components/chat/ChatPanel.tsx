...
export default function ChatPanel({ conversation, isStreaming, onSend, tools }: Props) {
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col bg-white">
      ...
      <ChatInput
        onSend={onSend}
        disabled={isStreaming || !websiteConnected}
        placeholder={
          websiteConnected ? undefined : 'Connect a website to use its tools'
        }
      />
    </div>
  );
