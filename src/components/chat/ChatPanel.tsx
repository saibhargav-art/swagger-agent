...
export default function ChatPanel({ conversation, isStreaming, onSend, tools, onOpenConversations }: Props) {
  ...
  const agentConnected = runtime.status === 'connected';
  const modelConfigured = runtime.modelProvider === 'openai'
    ? Boolean(runtime.openAiApiKey.trim() && runtime.openAiModel.trim())
    : Boolean(runtime.anthropicApiKey.trim() && runtime.anthropicModel.trim());
  const toolCount = tools.length;
  const title = conversation?.title ?? appName ?? 'AI Chat';
  const ready = agentConnected && websiteConnected && toolCount > 0;
  const missingReason = connectionMissingReason({
    agentConnected,
    modelConfigured,
    websiteConnected,
    toolCount,
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col bg-white">
      <div className="shrink-0 border-b border-slate-200 px-3 py-3 sm:px-5">
        <div className="mx-auto flex min-h-10 w-full max-w-3xl items-center gap-3">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            <Readiness label={runtime.modelProvider === 'anthropic' ? 'Claude' : 'OpenAI'} ready={agentConnected} />
      ...
      {!ready ? (
        <div className="border-b border-amber-100 bg-amber-50 px-3 py-2 sm:px-5">
          <div className="mx-auto flex w-full max-w-3xl items-center gap-2 text-xs text-amber-800">
            <AlertCircle size={15} className="shrink-0" />
            <span className="min-w-0 flex-1">{missingReason}</span>
...
function connectionMissingReason({
  agentConnected,
  modelConfigured,
  websiteConnected,
  toolCount,
  ...
  if (!modelConfigured) return 'Choose OpenAI or Claude and enter the API key before chatting.';
  if (!agentConnected) return 'Connect the local Strands agent before chatting.';
  if (!websiteConnected) return 'Connect a customer app before chatting.';
  if (toolCount === 0) return 'The connected app did not expose any WebMCP tools.';
  return 'Complete connections before chatting.';
}
